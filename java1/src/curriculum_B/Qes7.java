...
public class Qes7 {
	...
	public static void main(String[] args) {
		...
		// マップの生成
		Map<Integer, Map<String,Integer>> score_map = new HashMap<Integer, Map<String,Integer>>();
		// [概要]マップの中身を入力と紐づける処理
		...
		for (Integer Subject : score_map.keySet()) {
			double sum = (score_map.get(Subject).get("english") + score_map.get(Subject).get("math") +
					score_map.get(Subject).get("science") + score_map.get(Subject).get("social"));
			double avgs = sum/4;
		...
		// 平均点の出力処理
		System.out.println();
		System.out.println("英語の平均点は" + String.format("%.2f", english_avg) + "点です。");
